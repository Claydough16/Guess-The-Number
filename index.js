...
async function start() {
  ...
  if (isNaN(secretNumber) || secretNumber < min || secretNumber > max) {
    console.log('Please enter a valid secret number\n')
    process.exit()
  } else {
  ...
  while (true) {
    ...
    if (response.toLowerCase() === `n`) {
      ...
      attempts++; 
      console.log(`\nI guessed it right! The number you chose is ${guess}!\n`)
    ...
    if ((newResponse === `h` && secretNumber <= min) || (newResponse === `l` && secretNumber >= max)) {
